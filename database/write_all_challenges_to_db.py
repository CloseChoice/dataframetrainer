...
def run(port, dbname, password, user, host):
    ...
    for func in [
        ...
        static_example_func = transform_function_string(
            inspect.getsource(func.static_example)
        )
        expected_static_func = transform_function_string(
            inspect.getsource(func.expected_static)
        )
        cursor.execute(
            f"insert into challenges (initial_task, possible_solution, static_example, expected_static) values ('{initial_string}', '{transform_string}', '{static_example_func}', '{expected_static_func}')"
        )
        ...
        cursor.execute(
            "select * from challenges where id in (select max(id) from challenges)"
        )
