'''Get the lowest expense for each customer-type combination. The types shall be columns, while the customer should be the index. 
...
class Pivot2(BaseChallenge):
    ...
    def transform(df: pd.DataFrame, **kwargs) -> pd.DataFrame:
        df = df.pivot_table(index='Customer', columns='Type', values='Expense', aggfunc=np.min)
        return df
