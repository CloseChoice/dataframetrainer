...
df = data_frames(
    columns=[column("A", dtype=int), column("B", dtype=float)],
    rows=st.tuples(
        st.integers(),
...
def test_something(df):
    if (df["ActualDate"] < pd.to_datetime("2100-01-01")).all():
        ...
        tm.assert_series_equal(
            df["CalcEnd"].dt.month, df["ActualDate"].dt.month, check_names=False
        )
        tm.assert_series_equal(
            df["CalcEnd"].dt.year, df["ActualDate"].dt.year, check_names=False
        )
        tm.assert_series_equal(
            df["CalcEnd"], df["ActualDate"] + pd.offsets.MonthEnd(0), check_names=False
        )
